...
function getConfig(opts: Opts): string {
    ...
    if (!loc) {
        throw new Error("unable to find the home location");
    }
    if (loc === home) {
        return path.join(loc, ".project.json");
    }
...
function getArgs(opts: Opts): string[] {
    ...
    if (operation === Operation.Add) {
        if (opts.args.length !== 3) {
            throw new Error(`expected 2 arguments, got ${opts.args.length}-1`);
        }
        return opts.args.slice(1);
    }
    if (operation === Operation.Remove) {
        if (opts.args.length !== 2) {
            throw new Error(`expected 1 argument, got ${opts.args.length}-1`);
        }
    }
    return opts.args.slice(1);
}
